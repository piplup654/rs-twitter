...
pub fn get_api_credentials() -> Result<(String, String), Box<dyn std::error::Error>> {
    dotenv().ok();
    // implement better error support with match and stuff
...
impl Client {
    ...
    pub async fn authenticate(&mut self){
        ...
        let auth_request = reqwest::Client::new().post(AUTHORIZE_ENDPOINT)
            .header("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8")
            .header("Authorization", format!("Basic {}", client_string))
            .body(auth_body_encoded)
            .send()
            .await
            .unwrap().json::<helper_structs::AuthResponse>().await.unwrap();
...
mod tests {
    ...
    fn check_client_creation() {
        ...
        let api_secret = String::from("543");
